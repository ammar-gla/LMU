...
As they have for much of the post-pandemic recovery period, the headline labour market indicators for London offer a mixed bag.
<br><br>
The number of payrolled employees and workforce jobs continued to rise further above pre-pandemic levels. The employment rate reached its highest level since 2021, while the inactivity rate fell, which had been a cause of concern, fell in the latest data. In the wider UK, a lower number of inactive 50-64 year olds brought down the overall national inactivity rate.
<br><br>
Yet the unemployment rate appears to be on the rise again, while the claimant count rate remains elevated.
<br><br>
Cost-of-living also continues to be a concern as inflation remained very high, at 9.6% in October, with year-on-year real pay growth dipping further into negative territory. Where we have data at the UK level, annual regular pay growth was 6.9% in the private sector and 2.7% in the public sector, among the largest differences on record.
<br><br>
...
-   The total number of **workforce jobs** `r condi_text(wfj_d_quart,"increase")` by `r abs2(wfj_d_quart)`, or `r abs2(wfj_p_quart)`%, between the `r wfj_oldquarter` and `r  wfj_newquarter` quarter of `r wfj_year`, and was `r abs2(wfj_d_dec19)`, or `r abs2(wfj_p_dec19)`%, `r condi_text(wfj_d_dec19,"above")` pre-pandemic (December 2019) levels.
...
```{r wfj sector, echo=FALSE,fig.cap = wfjseccap, out.width='100%',fig.alt=paste0("Horizontal bar chart showing the change in the number of workforce jobs in separate bars for London and the UK, for each industry, from December 2019 to ",wfj_latest_month)}
  ...
  wjf_industry_bar <- wfj_stats %>%
    ...
    scale_fill_manual(values = rev(pal), aesthetics = "fill")+
    theme_set(theme_gla(gla_theme = "default", y_label_length=100)) + #GLA theme and removes lines below y-axis labels
    scale_y_continuous(limits = c(-12, 20), labels = dollar_format(prefix = "", 
                                                                   suffix = "%", 
                                                                   largest_with_cents = 1)) +
    theme(plot.margin = unit(c(1,1,1,1), "cm"))+
    scale_x_discrete(expand = c(0,0)) +
