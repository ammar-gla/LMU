...
```{r, main_figures ,echo=FALSE}
  ...
  # Paye
  paye_d_month <- value_form(paye_stats %>% filter( date_day == max(date_day) & geography_name=="London" & measure_name=="emps") %>% pull(d_change_mom),s= signif_thous)
  
  paye_p_month <- perc_form(100*paye_stats %>% filter( date_day == max(date_day) & geography_name=="London" & measure_name=="emps") %>% pull(p_change_mom),d = digits_perc)
  
  paye_previous_month <- paye_stats  %>% mutate(lag_month = lag(date_day, n = 1)) %>% mutate(lag_month2 = format(lag_month, '%B')) %>% filter(date_day == max(date_day)) %>% filter(row_number()==1) %>%  pull(lag_month2)
...
Unemployment in London fell to a record low (4.0%), as did the UK rate for the second month running (3.5%), but other headline labour market indicators continue to present a less positive outlook.
<br><br>
The inactivity rate remains elevated, with the rise on the quarter partially due to record-high rates of long-term illness nationally. The employment rate in London fell on the quarter and remains 1.7 percentage points below pre-pandemic peaks.
<br><br>
Vacancy numbers have been falling nationally in three consecutive quarters and online job posting numbers in London have also been on the decrease since peaking in early 2022.
<br><br>
Inflation on the year remains high, leading to a real-terms decrease in median pay in London despite a small monthly increase in nominal pay.
:::
...
```{r paye industry stats,echo=FALSE}
  ...
  lowest_growth_ind_paye <- paye_nuts1ind_stats %>% 
    filter(date_day == max(date_day) & geography_name == "London" & measure_name=="emps" & measure_value>=sector_thresh_paye & !is.na(p_change_feb20)) %>%
...
```{r paye industry region, echo=FALSE, fig.cap=paye_seccap, out.width='100%',fig.alt=paste0("Horizontal bar chart with separate bars for London and the UK showing the change in number of payrolled employees in each industry from February 2020 to ",paye_ind_last_month) }
  ...
  lon_large_inds_paye <- paye_nuts1ind_stats %>%
    filter( measure_name=="emps" & measure_value >=sector_thresh_paye 
  ...
  paye_emps_ind_feb20 <- paye_nuts1ind_stats %>%
    ...
    scale_y_continuous(limits = c(-0.10, 0.16), labels = percent_format(accuracy=1)) +
    theme(plot.margin = unit(c(1,1,1,1), "cm"))+
...
```{r regions employment, echo=FALSE,fig.cap = empcap, out.width='100%',fig.alt=paste0("Bar chart showing the employment rate for each of the regions and countries in the UK separately, with London highlighted, for ",lfs_last_month)}
  ...
  lfs_regions_charting <- lfs_regions %>% 
    filter(!(geography_name %in% c("England","United Kingdom")) & date_day==max(date_day) & value_type_name=="Level" & sex_name=="Total" & measures_name=="Unemployment rates use employment plus unemployment as a denominator; all other rates use population levels as a denominator.")
...
```{r workforce jobs trend, echo=FALSE,fig.cap = wfjcap, out.width='100%',fig.alt=paste0("Line chart showing the number of workforce jobs in London from January 2010 to ",wfj_latest_month)}
  ...
  LMU_plotly_settings(ch_name = wfj_time_chart,
                      ...
                      subtitle_text = 'paste0("Seasonally adjusted, latest data for ", wfj_latest_month)',
                      hover_mode = "x") %>% 
    hide_legend()
...
# Find industries with lowest and highest growth in recent data
  ...
  lowest_growth <- wfj_stats %>% filter(item_name == "total workforce jobs" & date_day == max(date_day) & geography_name == "London" & measures_name == "Value" & obs_value>=sector_thresh & !is.na(change_p_dec19_within_date_rank)) %>% slice_max(change_p_dec19_within_date_rank,n=3) 
  
  highest_growth <- wfj_stats %>% filter(item_name == "total workforce jobs" & date_day == max(date_day) & geography_name == "London" & measures_name == "Value" & obs_value>=sector_thresh & !is.na(change_p_dec19_within_date_rank)) %>% slice_min(change_p_dec19_within_date_rank,n=3) 
  
  lowest_growth_q <- wfj_stats %>% filter(item_name == "total workforce jobs" & date_day == max(date_day) & geography_name == "London" & measures_name == "Value" & obs_value>=sector_thresh & !is.na(change_p_quart_within_date_rank)) %>% slice_max(change_p_quart_within_date_rank,n=3) 
  
  highest_growth_q <- wfj_stats %>% filter(item_name == "total workforce jobs" & date_day == max(date_day) & geography_name == "London" & measures_name == "Value" & obs_value>=sector_thresh & !is.na(change_p_quart_within_date_rank)) %>% slice_min(change_p_quart_within_date_rank,n=3) 
...
```{r wfj sector, echo=FALSE,fig.cap = wfjseccap, out.width='100%',fig.alt=paste0("Horizontal bar chart showing the change in the number of workforce jobs in separate bars for London and the UK, for each industry, from December 2019 to ",wfj_latest_month)}
