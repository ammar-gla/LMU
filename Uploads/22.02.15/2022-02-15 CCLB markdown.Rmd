...
title:  |
    | 
    | Claimant Count data for London Boroughs
...
la_bubble_change_chart <- la_age_cc_data %>% 
  ...
  ggplot(aes(x = p_change_since_mar20,
             y = age_name,
             fill=as.factor(geo_cat),
             text = paste(geography_name,"\n",
                          age_name, "\n",
                          "Change: ", perc_form(p_change_since_mar20,d=0),"%", "\n",
                          paste0("Count, ",cc_newmonth_abr,": "), value_form(measure_value,s=signif_thous),"\n",
                          sep = ""))) +
  geom_point(aes(x=p_change_since_mar20, y=jit_age,size=1),
             shape=16,
             stroke=0,
             alpha=.5)+
...
ggplotly(la_bubble_change_chart,tooltip = "text")%>% 
  ...
  layout(title = list(text = paste0("<b>","Change in claimant count by local authority and age group","</b>",
         ...
         legend = list(font = list(size = 14),title=list(text="")),
         margin = list(l = 50)) %>% 
  highlight(color=highlight_colour,on = "plotly_click",
            off = "plotly_doubleclick",
            selectize=TRUE,opacityDim=0.9,
