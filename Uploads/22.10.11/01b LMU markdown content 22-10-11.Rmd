...
```{r,paye figures, echo=FALSE}
  ...
  paye_lastyearmonth <- paye_stats  %>% mutate(lag_month = lag(date_day, n = 12)) %>% mutate(lag_month2 = format(lag_month, '%B %Y')) %>% filter( date_day == max(date_day) & geography_name=="London" & measure_name=="emps") %>%  pull(lag_month2)
...
- The number of payrolled employees "Aged: 50-64" in London was `r condi_text(paye_age_5064_perc_lon,"up")` by `r abs2(paye_age_5064_perc_lon)`%, while the number of employees "`r paye_age_min_age_lon`" was unchanged.
...
```{r paye age region, echo=FALSE, fig.cap=paye_agecap, out.width='100%' }
  ...
  paye_emps_age_feb20 <- paye_nuts1age_stats %>%
    ...
    scale_y_continuous(limits = c(-0.01, 0.09), labels = percent_format(accuracy=1)) +
...
```{r claimant chart, echo=FALSE,fig.cap = claimcap, out.width='100%'}
  ...
  GLALineChart(y_var = proportion_of_resident_population_aged_16_64_estimates, x_var = date_day,
