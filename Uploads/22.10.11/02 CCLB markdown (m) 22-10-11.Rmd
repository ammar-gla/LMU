...
IMAGES <- paste0(here("IMAGES","MAIN"),"/")
...
ClaimantCountDownload <- function( data_series = "NM_162_1",
  ...
  if( save_intermediate == TRUE) {
    fwrite(cc_nomis_data, file = paste0(INTERMEDIATE,Sys.Date(),"_LA_",data_series,"_claimant.csv"))
  }
  
 return( cc_nomis_data) 
  ...
  claimant_count_stats <- ClaimantCountDownload( data_series = "NM_162_1", time_period = c("2019-01","latest"), save_intermediate = TRUE, line_or_bar = "bar")
...
- Focusing on the last month, in the period up to `r cc_newmonth`, the non-seasonally adjusted claimant count `r condi_text(cc_lon_d_mom,"rise")`  by `r abs2(cc_lon_d_mom)` (`r abs2(cc_lon_p_mom)`%). 
...
lon_cc_timechart <- lon_cc_data  %>%
  filter(date >= "2019-01-01" & measure_name == "claimant_count") %>%
  select(date,measure_value,geography_name) %>% 
...
la_cc_map_data_geo_claimant <- la_cc_map_data_geo %>% 
  ...
  # 
  # # Define bins in loop and produce the relevant charts
  # for (var in c("rate","count")) {
  #   if (var=="count") {
  #     var_desc <- "claimant_count"
  #     perc_ints <- FALSE}
  #   else if(var=="rate") {
  #     var_desc <- "claimants_as_a_proportion_of_residents_aged_16_64"
  #     perc_ints <- TRUE
  #   }
  #   
  #   assign("map_title",eval(parse(text=paste0("map_title_",var))))
  #   dataset_leaflet <- la_cc_map_data_geo_bins %>% filter(measure_name==var_desc)
  #   
  #   # Define bin cuts for interactive map
  #   map_bins <- bin_data %>% 
  #     filter(measure_name==var_desc) %>% 
  #     pull(bin_val)
  #   
  #   # Define categorical bin  intervals for ggplot
  #   bin_ints <- bin_data %>%
  #     filter(measure_name==var_desc & !grepl("NA",bin_int)) %>% # remove any with NA in interval
  #     arrange(bin_int) %>% 
  #     pull(bin_int)
  #   
  #   bin_ints_n <- length(bin_ints)
  ...
  # cc_ggplot_map <- la_cc_map_data_geo_bins  %>%
  #   filter(measure_name==var_desc) %>% 
  #   ggplot(aes(geometry = geometry,
  ...
  #   labs(title = paste0(gsub("<br>","\n",map_title)), # Convert from HTML to text
  #        subtitle = paste0("Latest data to ",cc_newmonth),
  #        caption = gsub("<br>","\n",map_cap))
  # 
  # save_GLA_plot(plot_name = paste0("cc_ggplot_map_",var))
  # 
  # #Run interactive Map
  # cc_leaflet_map <- leaflet_map_output(dataset=dataset_leaflet,
  #                    dt_var="measure_value",
  #                    palette=map_palette,
  #                    bins=map_bins,
  #                    title=map_title,
  ...
  #   
  #   
  #   assign(paste0("map_bins_",var),map_bins)
  #   assign(paste0("map_palette_",var),map_palette)
  #   assign(paste0("cc_leaflet_map_",var),cc_leaflet_map)
  #   assign(paste0("cc_ggplot_map_",var),cc_ggplot_map)
  #   
  #   remove(map_bins,map_palette,cc_leaflet_map,cc_ggplot_map,dataset_leaflet)
  #   
  # }
...
la_age_cc_data <- cc_changes_data %>% 
  filter(date==max(date) & measure_name=="claimant_count" & sex_name=="Total" & 
  ...
  select(c(date:p_change_since_mar20,is_la)) %>%  
...
```{r cc bubble chart bar rate by LA, echo=FALSE,fig.cap = la_bubble_cap, out.width='100%'}
  ...
  la_bubble_change_chart <- la_age_cc_data %>% 
    ...
    ggplot(aes(text = paste0(geography_name,"\n",
                            ...
                            paste0("Count, ",cc_newmonth_abr,": "), value_form(measure_value,s=signif_thous),"\n"))) +
    ...
    labs(title = paste0("Change in claimant count by local authority and age group"),
         subtitle = paste0("Non-seasonally adjusted, percentage change between March 2020 and ", cc_newmonth),
         caption = "\nSource:  Claimant count by sex and age (Nomis) – data is not seasonally adjusted.")+
    theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))
  ...
  ggplotly(la_bubble_change_chart,tooltip = "text")%>% 
    ...
    plotly::layout(title = list(text = paste0("<b>","Change in claimant count by local authority and age group","</b>",
                                              "<br>",
                                              "<sup>",
                                              paste0("Non-seasonally adjusted, percentage change between March 2020 and ", cc_newmonth),
                                              "</sup>",
                                              "<br>"),
                                font = list(size = 22),
                                y = .95, xref = "plot"),
