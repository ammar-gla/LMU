...
#     arrange(date, geography_name,sex_name,age_name)
  ...
  paye_online_path <- "rtisajan2022" #REMEMBER TO MAKE THIS DYNAMIC
  
  
  raw_paye <- download.file( url = "https://www.ons.gov.uk/file?uri=%2femploymentandlabourmarket%2fpeopleinwork%2fearningsandworkinghours%2fdatasets%2frealtimeinformationstatisticsreferencetableseasonallyadjusted%2fcurrent/rtisajan2022.xlsx",
  ...
  #### (a) Each quarter, process data on PAYE at borough level and merge with geographical codes. 
  #### NB: MUST BE UPDATED MANUALLY TO NEWER DATA  
  
  latest_paye_nuts1age_name <- "2022-01-18_raw_paye.xlsx"
  
    paye_age_stats <- readxl::read_excel(path = paste0(INPUT,latest_paye_nuts1age_name), sheet = "28. Employees (Age)", skip = 5) %>%
    mutate(new_date = dmy(paste0("01",Date))) %>% 
  ...
  paye_nuts1age_stats <- readxl::read_excel(path = paste0(INPUT,latest_paye_nuts1age_name), sheet = "32. Employees (NUTS1Age)", skip = 6) %>%
    mutate(new_date = dmy(paste0("01",Date))) %>% 
    ...
    mutate( 
      ...
      annual_change = case_when(new_date - min(new_date) < 365 ~ NaN,
                                new_date - min(new_date) >= 365 ~ 100*(emp_level - lag(emp_level, n= 12))/lag(emp_level, n=12)),
      age_group =paste0("Aged: ", age_group)) %>%
    clean_names()   %>%  rbind(paye_age_stats) %>% 
  ...
  latest_paye_locauth_name <- "2021-12-14_raw_paye.xlsx"

  paye_la_stats <- readxl::read_excel(path = paste0(INPUT,latest_paye_locauth_name), sheet = "19. Employees (LA)", skip = 7) %>% 
    mutate(new_date = dmy(paste0("01",Date))) %>% 
...
-   **Payrolled employees** `r condi_text(paye_d_month,"rise")` by around `r abs2(paye_d_month)`, or `r abs2(paye_p_month)`%, between `r paye_lastmonth` and `r paye_newmonth` and was `r abs2(paye_p_feb20)`% `r condi_text(paye_p_feb20,"above")` pre-pandemic (February 2020) levels.
-   **Unemployment rate** continued to fall from its pandemic peak but was still higher than the UK average. London's unemployment rate was estimated at `r unem_rate`% for the three months ending `r lfs_newmonth`, `r condi_text(unem_pp_year,"up")` `r abs2(unem_pp_year)` percentage points (pp) on the same period in the previous year. The UK average was `r unem_rate_uk`%.
-   The **employment rate** in London was estimated at `r emp_rate`% for the same period, `r condi_text(emp_pp_quart,"up")` `r abs2(emp_pp_quart)`pp on both the previous quarter and from a year earlier. London's employment rate was in line with the UK average (`r emp_rate_uk`%).
-   London's **inactivity rate** (the measure of those not looking and/or not available to work) was estimated at `r inact_rate`%. While this was `r condi_text(inact_pp_year,"up")` `r abs2(inact_pp_year)`pp on the previous year, the inactivity rate `r condi_text(inact_pp_quart,"rise")` on the previous quarter and remained lower than the UK-wide estimate of `r inact_rate_uk`%.
...
-   Relative to pre-pandemic levels (February 2020), the number of payrolled employees in London was `r condi_text(paye_d_feb20,"up")` by `r abs2(paye_d_feb20)` or `r abs2(paye_p_feb20)`%. This compares to `r condi_text(paye_p_feb20_uk,"noun")` of `r abs2(paye_p_feb20_uk)`% across the UK on average.
...
Recent changes in the number of payrolled employees vary by age group. Compared to pre-pandemic levels (February 2020):

- The number of payrolled employees "`r paye_age_max_age_lon`" in London was `r condi_text(paye_age_max_perc_lon,"up")` by `r abs2(paye_age_max_perc_lon)`% in `r newest_period_paye_age`, while the number of employees "`r paye_age_min_age_lon`" was `r condi_text(paye_age_min_perc_lon,"up")` by `r abs2(paye_age_min_perc_lon)`%.

-   For the UK overall, those two age groups were `r condi_text(paye_age_max_l_perc_uk,"up")` by `r abs2(paye_age_max_l_perc_uk)`% and unchanged, respectively.
...
paye_age_bars <- paye_nuts1age_stats %>%
  ...
  theme(plot.margin = unit(c(1,1,1,1), "cm"))+
  labs(title = "Payrolled employees, change since February 2020",
       subtitle = paste0("Latest data for period ", newest_period_paye_age),
       caption = "\nSource: HM Revenue and Customs – Pay As You Earn Real Time Information.\n\nNote: Estimates are based on where employees live.") +
  theme(plot.caption = element_text(color = rgb(166,166,166,maxColorValue = 255)))

ggsave( paste0(MARKDOWN_IMAGES,Sys.Date(),"_","paye_age_feb20.svg"), device = "svg", width = 8, height = 8, units = "in")
ggsave( paste0(MARKDOWN_IMAGES,Sys.Date(),"_","paye_age_feb20.png"), device = "png", width = 8, height = 8, units = "in")
...
There is considerable variation in the change in payrolled employees by London authority:

-   Compared to February 2020, the number of payrolled employees living in `r paye_la_max_area` was `r condi_text(paye_la_max_val,"up")` by `r abs2(paye_la_max_val)`% in `r newest_period_paye_la`, while the number living in `r paye_la_min_area` was `r condi_text(paye_la_min_val,"up")` by `r abs2(paye_la_min_val)`%.
...
-   The UK rate of economic inactivity was `r inact_rate_uk`%, `r condi_text(inact_pp_quart_uk,"noun")` of `r abs2(inact_pp_quart_uk)`pp on the previous quarter and `r condi_text(inact_pp_year_uk,"noun")` of `r abs2(inact_pp_year_uk)`pp on the year.
...
-   The number of claimants in London has `r condi_text(claim_d_year,"increase")` by `r abs2(claim_d_year)`, or `r abs2(claim_p_year)`%, on the year, compared to `r condi_text(claim_d_year,"noun")` of `r abs2(claim_d_year_uk)`, or `r abs2(claim_p_year_uk)`%, for the UK overall.
...
headline_rates <- lfs_stats %>% 
  ...
  mutate(num_le = round(num_le/1000,0),
         num_cq = round(num_cq/1000,0),
         num_cy = round(num_cy/1000,0),
         num_cq2 = condi_arrow(num_cq),
