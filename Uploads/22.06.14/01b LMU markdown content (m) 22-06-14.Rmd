...
```{r, main_figures ,echo=FALSE}
  ...
  wfj_oldquarter <-  wfj_stats %>% filter(date_day == (ymd(max(date_day)) %m+% - months(3))) %>% filter(row_number()==1) %>% pull(quarter_text)
...
- Nominal employee pay was `r condi_text(pay_lon_nom_p_change_yoy,t="up")` by `r abs2(perc_form(pay_lon_nom_p_change_yoy_cpih))`% on the year in London, resulting in `r condi_text(pay_lon_real_p_change_yoy_cpih,t="noun")` of `r abs2(perc_form(pay_lon_real_p_change_yoy_cpih))`% in real pay on the year.
...
```{r paye la stats,echo=FALSE}
  ...
  paye_la_max_val <- perc_form(paye_la_stats %>% filter(date_day == max(date_day)) %>% filter(p_change_feb20 == max(p_change_feb20)) %>% pull(p_change_feb20),d = digits_perc)
  
  paye_la_max_area <- paye_la_stats %>% filter(date_day == max(date_day)) %>% filter(p_change_feb20 == max(p_change_feb20)) %>% pull(area)
  
  paye_la_min_val <- perc_form(paye_la_stats %>% filter(date_day == max(date_day)) %>% filter(p_change_feb20 == min(p_change_feb20)) %>% pull(p_change_feb20),d = digits_perc)
  
  paye_la_min_area <- paye_la_stats %>% filter(date_day == max(date_day)) %>% filter(p_change_feb20 == min(p_change_feb20)) %>% pull(area)
  
  paye_la_count_below_feb20 <- paye_la_stats %>% filter(date_day == max(date_day)) %>% mutate(count_below_feb20 = sum(p_change_feb20<0)) %>% filter(row_number()==1) %>%  pull(count_below_feb20)
  
  paye_la_count_tot <- paye_la_stats %>% filter(date_day == max(date_day)) %>% add_count() %>% filter(row_number()==1) %>%  pull(n)
...
```{r wfj sector, echo=FALSE,fig.cap = wfjseccap, out.width='100%'}
  ...
  wjf_industry_bar <- wfj_stats %>%
    ...
    scale_fill_manual(values = rev(pal), aesthetics = "fill")+
    theme_set(theme_gla(gla_theme = "default", y_label_length=100)) + #GLA theme and removes lines below y-axis labels
    scale_y_continuous(limits = c(-12, 12), labels = dollar_format(prefix = "", 
                                                                   suffix = "%", 
                                                                   largest_with_cents = 1)) +
    theme(plot.margin = unit(c(1,1,1,1), "cm"))+
    scale_x_discrete(expand = c(0,0)) +
