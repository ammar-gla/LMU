...
la_cc_bar <- cc_changes_data %>%  
  ...
  scale_y_continuous(limits = c(0, 12000), labels = dollar_format(prefix = "", 
                                                                 suffix = "", 
                                                                 largest_with_cents = 1)) +
  theme(plot.margin = unit(c(1,1,1,1), "cm"))+
...
```{r cc bubble chart bar rate by LA, echo=FALSE,fig.cap = la_bubble_cap, out.width='100%'}
  ...
  max_p_change_axis_value <- ceiling(max(la_age_cc_data$p_change_since_mar20)/100)*100
  ...
  ggplotly(la_bubble_change_chart,tooltip = "text")%>% 
    ggla_plotly_settings()    %>% 
    layout(title = list(text = paste0("<b>","Change in claimant count by local authority and age group","</b>",
                                      "<br>",
                                      "<sup>",
