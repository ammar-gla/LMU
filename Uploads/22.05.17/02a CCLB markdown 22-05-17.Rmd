...
#################################################################
  ...
  NOMIS_API_KEY <- "0x01a88c6659d20042f087de2e585cdf3a07708983"
...
ClaimantCountDownload <- function( data_series = "NM_162_1",
  ...
  if( save_intermediate == TRUE) {
    fwrite(claimant_count_stats, file = paste0(INTERMEDIATE,Sys.Date(),"_LA_",data_series,"_claimant.csv"))
  }
...
```{r cc borough map, echo=FALSE}
  ...
  la_cc_map_data_geo <- la_cc_map_data %>%
    ...
    select(value,measure_name,GSS_CODE,tooltip)
  ...
  ## For rates map
  map_palette_rate <- rev(gla_pal(gla_theme="inverse", palette_type = "quantitative", main_colours = "red", n = 5))
...
```{r cc bubble chart bar rate by LA, echo=FALSE,fig.cap = la_bubble_cap, out.width='100%'}
  ...
  la_bubble_change_chart <- la_age_cc_data %>% 
    ...
    ggplot(aes(text = paste(geography_name,"\n",
                            age_name, "\n",
