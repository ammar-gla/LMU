...
Having improved significantly since the depths of the pandemic, the latest labour market statistics again indicate a slowdown in London’s recovery. 
<br><br>
The employment rate was down slightly on the previous quarter and the growth in payrolled employees has moderated in recent months.
<br><br>
In addition, inactivity is up on both the quarter and the year, while both the unemployment and the claimant count rates have recently levelled off.
<br><br>
Although nominal pay growth remains relatively healthy, many employees are still likely to see a decrease in pay in real terms as a result of high inflation.
:::
...
-   The **unemployment rate** was unchanged but still higher than the UK average. London's unemployment rate was estimated at `r unem_rate`%, unchanged on the quarter and `r condi_text(unem_pp_year,"up")` `r abs2(unem_pp_year)`pp from a year earlier. The UK average was `r unem_rate_uk`%.
...
```{r paye age region, echo=FALSE, fig.cap=paye_agecap, out.width='100%' }
  ...
  paye_age_bars <- paye_nuts1age_stats %>%
    ...
    scale_y_continuous(limits = c(-2, 8), labels = dollar_format(prefix = "", 
                                                                 suffix = "%", 
                                                                 largest_with_cents = 1)) +
...
-   The unemployment rate for London was estimated at `r unem_rate`%; this represents no change on the previous quarter and `r condi_text(unem_pp_year,"noun")` of `r abs2(unem_pp_year)`pp from a year earlier.
